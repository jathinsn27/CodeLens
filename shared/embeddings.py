...
def init_embeddings(model_path: str | None = None):
    ...
    try:
        ...
        print("WARNING: llama-cpp-python not installed. Embedding features disabled.")
        return
...
def _local_embed(text: str) -> list[float]:
    ...
    # Handle both list and nested list returns
    if isinstance(result, list) and len(result) > 0:
        if isinstance(result[0], list):
            return result[0]
        return result
    raise RuntimeError(f"Unexpected embedding result format: {type(result)}")
